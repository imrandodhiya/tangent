...
// ——— helper: normalize URL to https:// if missing ———
...
// ——— helper: quick URL-ish check (super light) ———
const looksLikeUrl = (v: string) => {
  if (!v) return true // optional fields
  try {
    // Allow bare domains too by normalizing first
...
export default function CreateMemberPage() {
  ...
  const validateForm = () => {
    ...
    urlFields.forEach((f) => {
      // allow empty; only error if present and not URL-like
      const val = (formData as any)[f]
      if (val && !looksLikeUrl(val)) {
        newErrors[f] = "Enter a valid URL"
      }
    })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/members", {
        ...
        body: JSON.stringify({
          ...formData,
          // normalize URLs right before sending
      ...
      if (response.ok) {
        ...
        setErrors({ submit: error.error || "Failed to create member" })
      }
    } catch (error) {
  ...
  return (
    <AuthGuard requiredPermissions={["member.create"]}>
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-2">Register a new bowling team member</p>
          </div>
        </div>
